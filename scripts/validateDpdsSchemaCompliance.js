...
const main = async () => {
    ...
    const validate = await ajv
        .compileAsync(JSON.parse(fs.readFileSync("schema/2.0.0/data-product-definition-schema.json"))).then((validate) => {
            let valid = true;
            const data = JSON.parse(fs.readFileSync("scripts/test-dpd.json"));
